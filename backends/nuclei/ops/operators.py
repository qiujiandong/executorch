...
from executorch.backends.nuclei.passes.passes_utils import (
...
from executorch.backends.nuclei.quantizer.quantization_configs import (
    CMSIS_SOFTMAX_SCALE,
...
@register_fake("nuclei::quantize_per_tensor")
...
@register_fake("nuclei::dequantize_per_tensor")
...
@register_fake("nuclei::quantized_add")
...
@register_fake("nuclei::quantized_mul")
...
@register_fake("nuclei::quantized_batch_matmul")
...
@register_fake("nuclei::quantized_linear")
...
def softmax_impl(
    ...
    if input.dtype != torch.int8:
        raise TypeError(
            f"nuclei.softmax: expected int8 input tensor, got {input.dtype}"
        )
    if output_zero_point != CMSIS_SOFTMAX_ZERO_POINT:
        raise ValueError(
            f"nuclei.softmax: expected output_zero_point {CMSIS_SOFTMAX_ZERO_POINT}, got {output_zero_point}"
        )
    ...
    if input_scale <= 0:
        raise ValueError(
            f"nuclei.softmax: derived non-positive input scale {input_scale}"
        )
...
@register_fake("nuclei::transpose")
...
@register_fake("nuclei::quantized_conv2d")
...
@register_fake("nuclei::quantized_depthwise_conv2d")
...
@register_fake("nuclei::quantized_transpose_conv2d")
...
@register_fake("nuclei::quantized_avg_pool2d")
...
@register_fake("nuclei::quantized_max_pool2d")
